...
void ClipGibVelocity (vec3_t vec)
{
	if (vec[0] < -300)
		vec[0] = -300;
	else if (vec[0] > 300)
...
{
	ThrowPhysicsGib(self, gibname, damage, type);
	return;
...
{
	ConvertToEntityEvent(self, EV_EXPLOSION, entityParms_t().PushByte(0));
}
...
void path_corner_touch (edict_t *self, edict_t *other, plane_t *plane, cmBspSurface_t *surf)
{
	...
	if ((next) && (next->spawnflags & 1))
	{
		...
		AddEntityEvent(other, EV_TELEPORT, entityParms_t().PushByte(1));
	}
...
void teleporter_touch (edict_t *self, edict_t *other, plane_t *plane, cmBspSurface_t *surf)
{
	...
	AddEntityEvent(other, EV_TELEPORT, entityParms_t().PushByte(0));
