...
void P_DamageFeedback (edict_t *player)
{
	...
	if ((level.time > player->pain_debounce_time) && !(player->flags & FL_GODMODE) && (client->invincible_framenum <= level.framenum))
	{
		...
		AddEntityEvent(player, EV_PAIN, entityParms_t().PushByte(l).PushByte(r));
	}
	...
	Vec4Set(client->ps.viewBlend, v[0], v[1], v[2], client->damage_alpha);
...
void P_FallingDamage (edict_t *ent)
{
	...
	if (delta < 15)
	{
		AddEntityEvent(ent, EV_FOOTSTEP, entityParms_t());
		return;
	}
	...
	if (delta > 30)
	{
		...
		{
			if (delta >= 55)
				AddEntityEvent(ent, EV_FALL, entityParms_t().PushByte(2));
			else
				AddEntityEvent(ent, EV_FALL, entityParms_t().PushByte(1));
		}
		...
		if (!deathmatch->floatVal || !((int)dmflags->floatVal & DF_NO_FALLING) )
			T_Damage (ent, Q_World, Q_World, dir, ent->s.origin, vec3Origin, damage, 0, 0, MOD_FALLING);
	}
	else
	{
		AddEntityEvent(ent, EV_FALL, entityParms_t().PushByte(0));
		return;
	}
}
	...
	if (waterlevel == 3)
	{
		...
		{	// drown!
				...
				&& current_player->health > 0)
			{
				...
				T_Damage (current_player, Q_World, Q_World, vec3Origin, current_player->s.origin, vec3Origin, current_player->dmg, 0, DAMAGE_NO_ARMOR, MOD_WATER);
			}
		}
	}
	else
	...
	if (waterlevel && (current_player->watertype&(CONTENTS_LAVA|CONTENTS_SLIME)) )
	{
		if (current_player->watertype & CONTENTS_LAVA)
		{
			...
			if (envirosuit)	// take 1/3 damage with envirosuit
				T_Damage (current_player, Q_World, Q_World, vec3Origin, current_player->s.origin, vec3Origin, 1*waterlevel, 0, 0, MOD_LAVA);
			else
				T_Damage (current_player, Q_World, Q_World, vec3Origin, current_player->s.origin, vec3Origin, 3*waterlevel, 0, 0, MOD_LAVA);
		}
		...
		if (current_player->watertype & CONTENTS_SLIME)
		{
			...
			{	// no damage from slime with envirosuit
				T_Damage (current_player, Q_World, Q_World, vec3Origin, current_player->s.origin, vec3Origin, 1*waterlevel, 0, 0, MOD_SLIME);
			}
		}
	}
}
...
void G_SetClientFrame (edict_t *ent)
{
	...
	bool	duck = (client->ps.pMove.pmFlags & PMF_DUCKED);
	bool	run = (xyspeed != 0);

	if (ent->s.modelIndex != 255)
		return;		// not in the player model

	ent->s.animation = ((run) ? 1 : 0) | ((duck) ? 2 : 0) | ((ent->groundentity) ? 4 : 0);
}
...
void ClientEndServerFrame (edict_t *ent)
{
	...
	{	// so bobbing only cycles when on ground
		if (xyspeed > 210)
			bobmove = 250.f / (ServerFrameFPS * 100);//0.25;
		else if (xyspeed > 100)
			bobmove = 125.f / (ServerFrameFPS * 100);//0.125;
		else
			bobmove = 62.5f / (ServerFrameFPS * 100); //0.0625;
	}
