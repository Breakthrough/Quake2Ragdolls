...
void target_explosion_explode (edict_t *self)
{
	...
	CreateEntityEvent(EV_EXPLOSION, entityParms_t().PushByte(0), self->s.origin, vec3Origin);
...
void use_target_changelevel (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if (deathmatch->floatVal && !( (int)dmflags->floatVal & DF_ALLOW_EXIT) && other != Q_World)
	{
...
{
	AddEntityEvent(self, EV_SPLASH, entityParms_t().PushByte(self->count).PushByte(self->sounds));
...
void SP_target_splash (edict_t *self)
{
	...
	Vec3Copy(self->movedir, self->s.angles);

	if (!self->count)
		self->count = 32;

	self->svFlags = SVF_EVENT;
	self->s.type |= ET_EVENT;
	self->freeAfterEvent = false;
