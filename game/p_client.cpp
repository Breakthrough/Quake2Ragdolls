...
void TossClientWeapon (edict_t *self)
{
	...
	if (item)
	{
		...
		drop = Drop_Physics_Item (self, item);
		self->client->v_angle[YAW] += spread;
...
void LookAtKiller (edict_t *self, edict_t *inflictor, edict_t *attacker)
{
	...
	if (attacker && attacker != Q_World && attacker != self)
	{
		Vec3Subtract (attacker->s.origin, self->s.origin, dir);
	}
	else if (inflictor && inflictor != Q_World && inflictor != self)
	{
...
void InitClientPersistant (gclient_t *client)
{
	...
	client->pers.inventory[ITEM_INDEX(FindItem("Slugs"))] = 99;
#else
...
void respawn (edict_t *self)
{
	if (deathmatch->floatVal || coop->floatVal)
	{
		...
		AddEntityEvent(self, EV_TELEPORT, entityParms_t().PushByte(0));
...
void spectator_respawn (edict_t *ent)
{
	...
	if (!ent->client->pers.spectator)  {
		// send effect
		AddEntityEvent(ent, EV_MUZZLEFLASH, entityParms_t().PushByte(MZ_LOGIN));
...
void ClientDisconnect (edict_t *ent)
{
	...
	// send effect
	AddEntityEvent(ent, EV_MUZZLEFLASH, entityParms_t().PushByte(MZ_LOGOUT));
...
void ClientThink (edict_t *ent, userCmd_t *ucmd)
{
	...
	if (ent->client->chase_target || ent->client->chaseEntity) {
