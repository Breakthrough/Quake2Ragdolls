...
{
	fire_bullet (self, start, dir, damage, kick, hspread, vspread, MOD_UNKNOWN, twister);
...
{
	fire_shotgun (self, start, aimdir, damage, kick, hspread, vspread, count, MOD_UNKNOWN, twister);
...
void M_WorldEffects (edict_t *ent)
{
		...
		if (!(ent->flags & FL_SWIM))
		{
			...
			{	// drown!
				if (ent->pain_debounce_time < level.time)
				{
					...
					if (dmg > 15)
						dmg = 15;
					T_Damage (ent, Q_World, Q_World, vec3Origin, ent->s.origin, vec3Origin, dmg, 0, DAMAGE_NO_ARMOR, MOD_WATER);
	...
	if ((ent->watertype & CONTENTS_LAVA) && !(ent->flags & FL_IMMUNE_LAVA))
	{
		if (ent->damage_debounce_time < level.time)
		{
			...
			T_Damage (ent, Q_World, Q_World, vec3Origin, ent->s.origin, vec3Origin, 10*ent->waterlevel, 0, 0, MOD_LAVA);
		}
	}
	if ((ent->watertype & CONTENTS_SLIME) && !(ent->flags & FL_IMMUNE_SLIME))
	{
		if (ent->damage_debounce_time < level.time)
		{
			...
			T_Damage (ent, Q_World, Q_World, vec3Origin, ent->s.origin, vec3Origin, 4*ent->waterlevel, 0, 0, MOD_SLIME);
		}
	}
