...
void Move_Final (edict_t *ent)
{
	...
	Vec3Scale (ent->moveinfo.dir, ent->moveinfo.remaining_distance / 0.1f, ent->velocity);
...
void Move_Begin (edict_t *ent)
{
	...
	frames = floor((ent->moveinfo.remaining_distance / ent->moveinfo.speed) / 0.1f);
	ent->moveinfo.remaining_distance -= frames * ent->moveinfo.speed * 0.1f;
	ent->nextthink = level.time + (frames * 0.1f);
...
void AngleMove_Final (edict_t *ent)
{
	...
	Vec3Scale (move, 1.0/0.1f, ent->avelocity);
...
void SP_func_plat (edict_t *ent)
{
	...
	Phys_SetBModelOnEntity(ent, GetBModelShape(ent->s.modelIndex-1));
...
//		gi.dprintf ("train_next: no next target\n");
	...
	if (ent->spawnflags & 1)
	{
		...
		AddEntityEvent(self, EV_TELEPORT, entityParms_t().PushByte(1));
		gi.linkentity (self);
		goto again;
	}
...
void SP_func_train (edict_t *self)
{
	...
	self->solid = SOLID_BSP;
	gi.setmodel (self, self->model);
	Phys_SetBModelOnEntity(self, GetBModelShape(self->s.modelIndex-1));
